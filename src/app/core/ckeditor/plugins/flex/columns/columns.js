...
export default class ColumnLayout extends BaseLayout {
  ...
  init() {
    ...
    // column ui
    const LAYOUT_COLUMN_1 = 'col-sm';
    editor.ui.componentFactory.add(
      LAYOUT_COLUMN_1, locale => {
        ...
        // Execute command.
        this.listenTo(btn, 'execute', () => cmd.execute(1));
    ...
    editor.plugins.get(Layout).registeredLayouts.push(
      editor.ui.componentFactory.create(LAYOUT_COLUMN_1)
    );

    // column ui
    const LAYOUT_COLUMN_2 = 'col-sm';
    editor.ui.componentFactory.add(
    ...
    function viewLayoutToModel() {
      ...
      function converter(evt, data, conversionApi) {
        ...
        if (!conversionApi.consumable.test(
          viewLayout,
          { name: true, attributes: 'columns', classes: 'row ms-0 me-0 ps-0 pe-0' }
        )) {
          return;
        }
